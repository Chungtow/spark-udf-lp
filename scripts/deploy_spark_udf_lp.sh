...
ssh "${HOST}" "docker exec spark hdfs dfs -mkdir -p ${HDFS_DIR} && docker exec spark hdfs dfs -put -f /tmp/${JAR} ${HDFS_DIR}/${JAR} && docker exec spark hdfs dfs -ls ${HDFS_DIR}/${JAR}"
ssh "${HOST}" "rm -f /tmp/${JAR}"

echo "[2/3] 注册 Hive 永久函数（清单 ${MANIFEST}）"
while IFS='|' read -r fname fclass; do
  case "${fname}" in ""|\#*) continue ;; esac
  echo "  -> ${fname} AS ${fclass}"
  ssh "${HOST}" "docker exec -i spark /opt/spark/bin/spark-sql --master yarn --name spark-udf-register" <<SQL
CREATE OR REPLACE FUNCTION ${fname} AS '${fclass}'
  USING JAR '${HDFS_URL}${HDFS_DIR}/${JAR}';
SQL
done < "${MANIFEST}"
